 ...
 ******************************************************/
 ...
 const requiredDjsVersion = "14.2.0";
 ...
 function PrintError(type) {
   ...
   switch (type) {
     ...
     case MsgType.DUPLICATE_BUTTON_ID: {
       warn(
         format(
           'Zgłoś się na na discordzie https://discord.gg/GuGeN9BwRV na kanale dbm-pomoc: "%s"',
           arguments[1],
         ),
       );
       break;
 ...
 Bot.PRIVILEGED_INTENTS = GatewayIntentBits.GUILD_MEMBERS | GatewayIntentBits.GUILD_PRESENCES;
 ...
 Bot.initBot = function () {
   ...
   this.hasMemberIntents = (options.intents & GatewayIntentBits.GuildMembers) !== 0;
   ...
   for (let i = 0; i < parameters.length; i++) {
     ...
     if (name) {
       if (!existingNames[name]) {
         ...
         paramsData.min_value = paramsData.minvalue
         paramsData.max_value = paramsData.minvalue
 ...
 Bot.setGlobalCommands = function (commands) {
   this.bot.application?.commands?.set?.(commands).then(function() {}).catch((err) => {
     console.error(err);
   })
 };
 ...
 Actions.generateButton = function (button, cache) {
   const style = button.url ? "5" : button.type;
   const buttonData = {
     type: 2,
